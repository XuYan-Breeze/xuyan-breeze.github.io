  ...
  const workExperience = [
    {
      ...
      achievements: [
        ...
        "自研并开发大气波导反演系统（Python）：开发基于 ERA5 与掩星大气廓线大气波导反演脚本（Python），实现接地/悬空波导自动识别与绘图，开发基于 ERA5 数据评估云遥掩星同化数值模式大气波导精度脚本（Python），相关技术成果已授权发明专利-《基于掩星同化的数值模式大气波导预报方法及装置》",
        "作为技术骨干参与军工重点项目，完成项目GNSS 掩星质量评估程序（Fortran）、自动化脚本（Python）和软件GUI（tkinter）的开发，具备银河麒麟（国防版）服务器本地化部署经验，可在受限内网环境完成编译、依赖打包与离线安装，撰写验收文档（技术总结、质量评估、测试大纲/报告等），项目一次性通过专家评审并获“优秀”评级",
        "负责国际前沿的全球尺度中尺度大气-电离层耦合模式（GSMWAM-IPE）的调试与开发工作，该模式整合了 WAM 大气模式、IPE电离层模式、NEMS 耦合框架和 ESMF/MPI 并行计算技术，熟练使用Claude Code AI编程智能体解决跨学科（大气物理、电离层电磁场、空间天气）耦合难题，深入底层源码，精确定位并修复30+个核心错误，编写Python自动化配置脚本实现一键部署",
      ]
    }
  ];
